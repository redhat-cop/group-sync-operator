...
* Under "permissions and events" the app will need read-only access to the "Members" permission in the "Organization" section. NOTE: If you enable `MapByScimId` this permissions needs to be _Read & Write_, although, in-fact the operator only does read-only operations. The reason for this is the use of the v4 graphql api-endpoint.
