...
func (l *allOpenShiftGroupLister) ListGroups() ([]string, error) {
	...
	allGroups := &userv1.GroupList{}
	err = l.client.List(context.TODO(), allGroups, &client.ListOptions{LabelSelector: hostSelector})
	if err != nil {
		return nil, err
	}
...
func (l *openshiftGroupLister) ListGroups() ([]string, error) {
	...
	for _, name := range l.whitelist {
		...
		group := &userv1.Group{}
		err := l.client.Get(context.TODO(), types.NamespacedName{Name: name, Namespace: ""}, group)
		if err != nil {
			return nil, err
		}
